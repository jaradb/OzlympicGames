...
public class Official extends Person {
	...
	public String confirmGameResults(String gameType, int roundNumber, Athlete firstPlaceWinner,
		...
		report += "In " + gameType + " (round " + roundNumber + ") overseen by referee " + getName() + " (ID: "
				+ getUniqueID() + "), the results were -- \n";

		report += "1 (First place): " + firstPlaceWinner.getName() + " (ID: " + firstPlaceWinner.getUniqueID()
				+ "), age " + firstPlaceWinner.getAge() + " representing the state of "
				+ firstPlaceWinner.getRepresentingState().toString() + " with a current total of "
				+ firstPlaceWinner.getCurrentPointCount() + " point(s), finished in "
				+ firstPlaceWinner.getLastTimeRecorded() + " seconds.\n";

		report += "2 (Second place): " + secondPlaceWinner.getName() + " (ID: " + secondPlaceWinner.getUniqueID()
				+ "), age " + secondPlaceWinner.getAge() + " representing the state of "
				+ secondPlaceWinner.getRepresentingState().toString() + " with a current total of "
				+ secondPlaceWinner.getCurrentPointCount() + " point(s), finished in "
				+ secondPlaceWinner.getLastTimeRecorded() + " seconds.\n";

		report += "3 (Third place): " + thirdPlaceWinner.getName() + " (ID: " + thirdPlaceWinner.getUniqueID()
				+ "), age " + thirdPlaceWinner.getAge() + " representing the state of "
				+ thirdPlaceWinner.getRepresentingState().toString() + " with a current total of "
				+ thirdPlaceWinner.getCurrentPointCount() + " point(s), finished in "
				+ thirdPlaceWinner.getLastTimeRecorded() + " seconds.\n";
		...
		for (Athlete athlete : remainingCompetitors) {
			...
			report += remainingPositionsStartPoint + ": " + athlete.getName() + " (ID: " + athlete.getUniqueID()
					+ "), age " + athlete.getAge() + " representing the state of "
					+ athlete.getRepresentingState().toString() + " with a current total of "
					+ athlete.getCurrentPointCount() + " point(s), finished in " + athlete.getLastTimeRecorded()
					+ " seconds.\n";
		}
